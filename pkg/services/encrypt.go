...
func Encrypt(filePaths []string, keysFilePath string) {
	...
	for _, filePath := range filePaths {
		...
		// check if public already exists in file
		// isPublicKeyExistsInEnvFile := helpers.IsPublicKeyExistsInEnvFile(parsedEnvVars, fileName)
		...
		var eciesKey *eciesgo.PrivateKey
		...
		if containsEncryptedVar && !isPrivateKeyExistsForThisFile {
			...
			eciesKey = cryptography.GenerateKeyForECIESFromExistingKey(helpers.GetPrivateKeyFromKeysFile(keysFileParsed, fileName))
		} else if !containsEncryptedVar && isPrivateKeyExistsForThisFile {
			eciesKey = cryptography.GenerateKeyForECIESFromExistingKey(helpers.GetPrivateKeyFromKeysFile(keysFileParsed, fileName))
		} else if !containsEncryptedVar && !isPrivateKeyExistsForThisFile {
			eciesKey = cryptography.GenerateKeyForECIES()
			writePrivateKey = true
		}

		helpers.WriteEnvFile(string(content), filePath, eciesKey.PublicKey)

		if writePrivateKey {
			helpers.WritePrivateKeyToKeysFile(eciesKey.Hex(), fileName, keysFilePath)
		}
	}
}
